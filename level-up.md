...
두 기술 모두 **짝힘**을 활용하는 테크닉이다. 짝힘이란 서로 반대 방향으로 힘을 보내서 합력을 만들어 버티는 원리를 말한다.

### 5-1. 레이백

![레이백](images/level-up/layback.jpg)

손을 당기고 발을 밀면서 버티는 자세이다. 주로 손홀드가 수직으로 되어 있어서 그립을 강하게 가져갈 수 없을 때 사용한다.

- **방법**: 손으로 홀드를 당기고 발로 벽(또는 홀드)을 밀어 짝힘으로 버틴다
- **핵심**:
  - **발을 펴야** 밀어내는 힘이 잘 실린다
  - **손(팔)도 펴야** 팔에 힘을 덜 들일 수 있다
- **발 위치에 따른 차이**:
  - 발이 **높으면** 짝힘이 효과적으로 작용한다
  - 발이 **낮으면** 짝힘이 완벽하게 이루어지지 않으므로, **코어 근육**을 추가하여 보완해야 한다
- **상황**: 수직 방향의 홀드(사이드풀 등)를 잡아야 할 때 활용

### 5-2. 게스통

![게스통](images/level-up/gastons.jpg)

팔과 손바닥을 사용하여 몸 쪽으로 당기는 일반적인 동작과 달리, **바깥쪽으로 밀어내는** 독특한 등반 기술이다. 마치 엘리베이터 문을 가운데에서 억지로 열려고 하는 동작과 같다.

- **방법**: 손바닥을 바깥쪽으로 향하게 하고, 엄지손가락을 아래로 향한 자세로 홀드를 바깥쪽으로 밀어낸다
- **핵심**: 게스통을 하는 손의 **반대발**을 밀어주는 것이 일반적이다 (더블 게스통은 예외)
- **상황**: 홀드가 서로 마주 보는 방향으로 배치되어 있어, 바깥으로 밀어내야 버틸 수 있는 경우에 활용

---

## 6. 힐훅 & 토훅

> 초급반 12. 힐훅 & 토훅과 동일한 내용이므로 생략한다.

---

## 7. 데드포인트(원리/활용)

> 데드포인트 기본 개념은 초급반 7. 데드포인트를 참고한다. 여기서는 골반 활용 원리를 다룬다.

데드포인트는 골반을 뒤로 뺐다가 순간적으로 벽에 밀착시키는 동작이다. **골반을 벽에 순간적으로 붙이는 것**이 핵심이다.

### 7-1. 기본 동작 원리

- **준비**: 팔을 펴고 무게중심을 아래로 내려 릴리즈 상태를 만든다
- **동작**: 골반만 뒤로 뺐다가 벽쪽으로 집어 넣으면서, 코어의 힘으로 나머지 상체가 **웨이브를 그리듯이** 튕겨서 따라오게 한다
- **마무리**: 상체가 올라오는 타이밍에 다음 홀드를 잡는다

### 7-2. 손홀드 상태에 따른 차이

- 손홀드가 **좋을 때**: 손으로 당기는 힘을 함께 사용하여 데드포인트를 보조할 수 있다
- 손홀드가 **나쁠 때**: 당기는 힘을 사용할 수 없으므로, 골반 테크닉에 더 의존해야 한다

### 7-3. 방향이 있는 경우

다음 홀드가 정면 위가 아니라 **오른쪽 위** 또는 **왼쪽 위**처럼 방향이 있는 경우, 골반을 벽에 붙이면서 동시에 **무게중심을 해당 방향으로 이동**시켜야 한다.
