...
### 기본 개념
- **손바닥으로 홀드를 누르면서** 등반하는 기술
- 홀드를 잡아당기는 것이 아닌 **밀어서 몸을 올리는** 동작
- 수영장에서 나올 때 손바닥으로 바닥을 짚고 몸을 밀어 올리는 동작과 유사하다
- 맨틀링(mantling)이라고도 하며, 홀드의 윗면이나 볼륨 위를 눌러 올라서는 상황에서 사용된다

### 손 자세
- 손바닥을 홀드 위에 놓고 **아래로 누르며** 몸을 밀어 올린다
- **손가락이 벽 바깥쪽을 향하도록** 손을 놓아야 한다
  - 손가락이 벽 안쪽을 향하면 밀어 올릴 때 손가락에 과도한 부하가 걸려 **부상 위험**이 있다
  - 손가락이 바깥쪽을 향하면 손목과 팔이 자연스러운 각도를 유지할 수 있다
- 손바닥의 **접촉 면적을 최대한 넓혀** 마찰력을 확보한다
- 초크를 충분히 발라 손바닥의 미끄러짐을 방지한다

### 동작 순서
1. **홀드를 손바닥으로 누른다**
   - 손가락이 벽 바깥쪽을 향하도록 손을 돌린다
   - 손바닥 전체가 홀드에 접촉하도록 넓게 짚는다
2. **팔을 펴면서 몸을 밀어 올린다**
   - 삼두근(팔 뒤쪽 근육)을 사용하여 몸을 위로 밀어 올린다
   - 푸쉬업(팔굽혀펴기)과 유사한 근육을 사용한다
3. **손이 있던 홀드에 발을 올린다**
   - 몸이 충분히 올라온 상태에서 손이 짚고 있던 홀드에 발끝을 올려놓는다
4. **발로 일어선다**
   - 무게중심을 발 위로 이동시키고 다리 힘으로 일어선다
   - 팔은 다음 홀드를 잡거나 벽을 짚어 균형을 잡는다

### 힘 사용의 원리
- 클라이밍은 대부분 **당기는 동작**(이두근, 등근육)이지만, 푸쉬는 **미는 동작**(삼두근, 어깨)을 사용한다
- 당기는 근육이 지쳐있을 때 미는 근육을 사용할 수 있어 **체력 분배에 유리**하다
- 홀드를 잡기 어려운 상황에서도 손바닥으로 눌러 진행할 수 있다

### 주의점
1. **손가락 방향 필수 확인**
   - 반드시 손가락이 **벽 바깥쪽**을 향하도록 한다
   - 잘못된 손가락 방향은 손목과 손가락 부상의 주요 원인이 된다

2. **무게중심 이동이 핵심**
   - 몸을 밀어 올린 후 무게중심을 발 위로 완전히 넘겨야 한다
   - 무게중심이 넘어가지 않으면 팔로만 버텨야 해서 금방 지친다

3. **팔을 완전히 펴기**
   - 푸쉬 동작 시 팔을 완전히 펴야 뼈로 지지할 수 있다
   - 팔이 굽혀진 상태로 유지하면 근육에 부담이 집중된다

### 사용 시기
1. **홀드의 윗면을 사용해야 할 때**
   - 홀드를 잡기보다 눌러야 하는 형태일 때
2. **볼륨이나 넓은 홀드 위로 올라서야 할 때**
   - 맨틀링이 필요한 탑아웃(top-out) 상황
3. **잡을 수 있는 홀드가 없지만 누를 수 있는 면이 있을 때**
   - 슬로퍼나 평평한 면을 활용해야 할 때
...
#### 기본 개념
- **뒤꿈치를 홀드에 걸어서** 몸을 끌어올리거나 고정하는 기술
- 뒤꿈치의 **중앙이 아닌 복숭아뼈 부분**을 사용하여 홀드에 건다
  - 복숭아뼈 부분이 홀드에 더 잘 걸리고, 힘 전달이 효율적이다
- **햄스트링**(허벅지 뒤쪽)과 **둔근**(엉덩이) 등 큰 근육을 사용하여 강한 힘을 줄 수 있다

#### 동작 방법
1. **뒤꿈치를 홀드에 건다**
   - 복숭아뼈 부분이 홀드에 걸리도록 발을 위치시킨다
   - 뒤꿈치를 **벽과 홀드에 밀착**시켜 안정적으로 건다
2. **무릎을 벽에 붙이면서 진행한다**
   - 무릎을 벽 쪽으로 당기면서 몸을 끌어올린다
   - 무릎이 벽에서 떨어지면 힘이 분산되고 뒤꿈치가 빠지기 쉽다
3. **다음 홀드를 잡는다**
   - 햄스트링과 코어를 이용하여 몸을 벽에 붙인 상태에서 손을 이동한다
4. **발가락 지지로 전환한다**
   - 다음 홀드를 잡았으면 힐훅 포지션을 **유지하지 않고** 발가락으로 지지하는 것으로 전환한다
   - 힐훅을 오래 유지하면 햄스트링에 과도한 부하가 걸린다

#### 사용 시기
1. **오버행에서 몸이 벽에서 떨어지는 것을 방지할 때**
   - 힐훅으로 하체를 벽에 고정하여 팔의 부담을 줄인다
2. **높은 발홀드를 적극적으로 활용할 때**
   - 발가락보다 뒤꿈치가 더 쉽게 걸리는 홀드에서 사용
3. **몸의 회전(바나나 도어)을 방지할 때**
   - 한쪽 방향으로 몸이 돌아가려는 것을 힐훅으로 잡아준다

#### 주의점
1. **낙법 주의 — 머리부터 떨어질 위험**
   - 힐훅이 걸린 상태에서 손을 놓치면 **몸이 뒤집혀 머리부터 떨어질 수 있다**
   - 힐훅 사용 시 항상 낙법을 의식하고, 떨어질 때는 즉시 힐훅을 빼야 한다
   - 높은 위치에서의 힐훅은 특히 주의한다

2. **뒤꿈치를 벽과 홀드에 밀착**
   - 뒤꿈치가 홀드에서 떨어져 있으면 갑작스럽게 빠질 수 있다
   - 벽과 홀드에 밀착시켜 안정적으로 유지한다

3. **무릎 부상 방지**
   - 골반에 가까운 위치에서 발을 바깥으로 돌리면서 힘을 주면 **무릎 인대(LCL) 부상** 위험이 있다
   - 충분한 워밍업과 하체 스트레칭이 필수적이다

4. **햄스트링 부상 방지**
   - 힐훅은 햄스트링에 강한 부하가 걸리는 동작이다
   - 워밍업 없이 강하게 당기면 햄스트링 파열이 발생할 수 있다
...
#### 기본 개념
- **발등을 이용하여** 홀드에 걸치는 기술
- 발끝을 세워서 **발등(발가락 윗면~신발 끈 부분)**으로 홀드를 감싸듯이 건다
- 힐훅에 비해 **정강이 앞쪽의 작은 근육**을 사용하므로 강한 힘을 내기는 어렵다
- 주로 **몸을 잠시 고정**하거나 **흔들림(스윙)을 방지**하는 데 사용한다

#### 동작 방법
1. **발끝을 세워서 홀드에 건다**
   - 발등이 홀드의 튀어나온 부분에 걸리도록 위치시킨다
   - 발목에 힘을 주어 발끝을 위로 당기듯 홀드를 잡는다
2. **무릎을 펴서 텐션을 유지한다**
   - **무릎을 피는 것이 핵심** — 무릎이 굽혀지면 홀드에 전달되는 힘이 약해진다
   - 다리 전체로 당기는 느낌을 유지한다
3. **코어를 긴장시켜 몸을 안정시킨다**
   - 토훅으로 하체를 고정한 상태에서 손을 이동한다

#### 사용 시기
1. **오버행이나 루프에서 스윙을 방지할 때**
   - 몸이 흔들리는 것을 토훅으로 잡아준다
2. **아레트(모서리)에서 바나나 도어를 방지할 때**
   - 몸이 벽에서 회전하는 것을 막는다
3. **힐훅을 걸기 어려운 홀드에서**
   - 홀드의 형태에 따라 발등이 더 잘 걸리는 경우에 사용

#### 주의점 — 걸 때보다 뺄 때가 중요
1. **토훅을 뺄 때 스윙에 대비한다**
   - 토훅이 하체를 잡아주고 있기 때문에 빼는 순간 **몸이 크게 흔들릴 수 있다**
   - 빼기 전에 **코어를 단단히 긴장**시키고 손과 반대쪽 발이 안정적인지 확인한다
2. **한 발씩 천천히 뺀다**
   - 갑자기 빼면 반동으로 벽에서 떨어질 수 있다
   - 빼고 난 후 발이 갈 다음 위치를 미리 계획한다
3. **급하게 빼지 않는다**
   - 토훅을 빼면서 동시에 큰 동작을 하면 불안정해진다
   - 작은 스텝으로 나누어 발을 이동시킨다
...
| 구분 | 힐훅 | 토훅 |
|------|------|------|
| 사용 부위 | 뒤꿈치 (복숭아뼈 부분) | 발등 (발가락 윗면) |
| 사용 근육 | 햄스트링, 둔근 (큰 근육) | 정강이 앞쪽 근육 (작은 근육) |
| 힘의 강도 | 강한 힘 가능 | 상대적으로 약함 |
| 주요 용도 | 몸 끌어올리기, 고정 | 스윙 방지, 일시적 고정 |
| 주요 위험 | 머리부터 떨어짐, 햄스트링 부상 | 뺄 때 스윙으로 인한 추락 |
